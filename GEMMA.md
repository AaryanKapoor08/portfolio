...
## Branch: `main` — Engineer Edition

> **What is this?** A gated implementation system. Gemma acts as a senior engineer who builds the project for you — phase by phase, ensuring high quality and professional standards. Gemma writes the code, you review and approve.
...
- `gemma/Gemma_guide.md` — engineer rules, 13 habits, red lines (store in memory on first load)
- `gemma/ProjectSummary.md` — architecture, models, structure
- `gemma/Progress.md` — current phase and state

Load on demand only:
- `gemma/BuildFlow.md` — when entering a new phase or running `/phase-check`
- `gemma/G0_questionnaire.md` — only while G0 is incomplete

Operate in **Engineer Mode** at all times. No exceptions.
...
- Gemma **verifies** conditions before declaring a gate passed
...
- Any gemma/ file contains unfilled placeholders: `[PLACEHOLDER]`, `[TO_BE_FILLED]`, `[G0.6 fills]`, `[NAME]`, `[DATE]`, `[PROJECT_SCOPES]`, `[TDD_TARGETS]`, `[DOCKER_PHASE]`, `[CI_PHASE]`, `[PROJECT_RED_LINES]`, `[APP_NAME]`, `[FILLED BY G0.6]`
- `_fill_manifest.md` contains bracketed placeholder values

**If G0 incomplete →** load `gemma/G0_questionnaire.md`, run from earliest incomplete sub-gate.
...
1. Read `gemma/Progress.md` — ALL checkboxes for the phase must be `[x]`
...
5. Gemma demonstrates results — never accept claims without proof

Phase-specific proof requirements live in `gemma/BuildFlow.md` under each phase's **Proof** line.
...
Gemma NEVER checks a box without demonstrating the condition. "Done" → provide proof.
